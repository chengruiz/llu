...
template<typename T> constexpr T median(T a, T b, T c) {
...
template<typename Derived> auto unit(const Eigen::MatrixBase<Derived> &x) {
...
template<typename T> constexpr T ceilDiv(T x, T y) {
  static_assert(std::is_integral<T>());
  return (x + y - 1) / y;
}

template<typename T> constexpr T pMod(T dividend, T divisor) { // positive mod, divisor must be positive
  static_assert(std::is_integral<T>());
